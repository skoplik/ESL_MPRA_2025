...
output_dir      = "/ESL/ESL_MPRA/Figure_7/outputs/regenerated_panels"
...
# --- Figure S9C ---------------------------------------------------------------
# COLQ exon 5 has no variant passing BOTH mingap >= 0.25 and the ANOVA/BH filter
# (48 pass mingap; none survive correction), so it is absent from the sig+mingap
# set and the family would silently be skipped. Inject the motif-disruption
# subject so the panel renders in the same style as the Figure 7D families.
# Same precedent as plot_recurring_vars_cell_type_specific_fixed.py (COLQ added manually).
COLQ_HIGHLIGHT_REF = "55769"                      # chr3:15479074:G>T, snp 43:C>A
COLQ_EVENT_ID      = "chr3:15478957-15479117:-"   # event_id_161 (the column the filter uses)
sig_mingap_refs |= {COLQ_HIGHLIGHT_REF}
sig_mingap_eids |= {COLQ_EVENT_ID}
print("[INFO] injected COLQ highlight variant %s" % COLQ_HIGHLIGHT_REF)
...
df["event_id_161"] = df["event_id_161"].astype(str)
df = df[df["event_id_161"].isin(sig_mingap_eids)]
