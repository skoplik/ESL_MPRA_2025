...
allseq_file = "/ESL/Figures_SK/General_preprocessing/output_03_16_2026/03_16_2026_1e-2_ALL_WTS_VARS_NO_DELTAS.csv"
clinvar_path = "/ESL/Figures_SK/redo_pop_variants/clinvar_updated_7_15_25/swarm_delta_logit.csv"

output_plot = "/ESL/Figures_SK/Disease_relevant_genes/SFARI/sfari_overlap_clinvar_stripplot_boxplot.pdf"
...
# === Box/whisker overlay ===
n_exons = len(sfari_event_exons)
box_data = [
    variants[variants["x_pos"] == i]["avg_delta_logit"].dropna().values
    for i in range(n_exons)
]

ax.boxplot(
    box_data,
    positions=list(range(n_exons)),
    widths=0.45,
    showfliers=False,
    patch_artist=True,
...
print(f"Saved SFARI overlap + ClinVar stripplot (with boxplot) to {output_plot}")
