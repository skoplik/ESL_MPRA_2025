...
# === Average measured Δlogit pooled (across cell lines) ===
...
plot_corr(merged_dist100, "SpliceAI vs Measured (MMSplice subset, ≤100 nt)", "spliceai_vs_exp_dlogit_mmsplice_dist100.pdf")
...
plot_corr(merged_all_dist100, "SpliceAI vs Measured (all variants, ≤100 nt)", "spliceai_vs_exp_dlogit_all_dist100.pdf")

# === Combined Plot ===
subset_mm = merged_dist100[["delta_spliceai_logit", "avg_delta_logit_pooled"]].dropna()
subset_all = merged_all_dist100[["delta_spliceai_logit", "avg_delta_logit_pooled"]].dropna()

plt.figure(figsize=(4, 4))

# All variants as purple transparent dots
sns.scatterplot(
    data=subset_all,
    x="delta_spliceai_logit",
    y="avg_delta_logit_pooled",
...
# MMSplice subset as black transparent dots on top
sns.scatterplot(
    data=subset_mm,
    x="delta_spliceai_logit",
    y="avg_delta_logit_pooled",
    ...
    color="black",
    edgecolor="none",
    label="MMSplice subset"
)

lo = min(min(subset_all.min().min(), subset_mm.min().min()), -5)
hi = max(max(subset_all.max().max(), subset_mm.max().max()), 5)
plt.plot([lo, hi], [lo, hi], color="grey", linestyle="--", linewidth=1)
...
if len(subset_all) > 1:
    r_all, _ = pearsonr(subset_all["delta_spliceai_logit"], subset_all["avg_delta_logit_pooled"])
    n_all = len(subset_all)
else:
    r_all, n_all = np.nan, 0

if len(subset_mm) > 1:
    r_mm, _ = pearsonr(subset_mm["delta_spliceai_logit"], subset_mm["avg_delta_logit_pooled"])
    n_mm = len(subset_mm)
else:
    r_mm, n_mm = np.nan, 0

plt.title(f"SpliceAI vs Measured Δlogit(PSI)\nAll: r={r_all:.2f}, n={n_all:,} | MMSplice: r={r_mm:.2f}, n={n_mm:,}")
plt.xlabel("SpliceAI Δlogit(PSI)")
plt.ylabel("Measured Δlogit(PSI)")

# Legend with solid markers (ignore alpha from scatter)
handles, labels = plt.gca().get_legend_handles_labels()
for h in handles:
    h.set_alpha(1)
plt.legend(handles, labels, frameon=False, loc="upper left", markerscale=2)

outpath = os.path.join(outdir, "spliceai_vs_exp_dlogit_combined_dist100.pdf")
plt.tight_layout()
plt.savefig(outpath, dpi=300)
plt.close()
