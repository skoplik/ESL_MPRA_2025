...
def _plot_swarm_shared(df, ax, y_col, title, xlabel):
    ...
    for bin_code in sorted(df['bin_code'].dropna().unique()):
        ...
        if len(y_finite) > 1:
            kde = gaussian_kde(y_finite)
            densities = np.where(np.isfinite(y_vals), kde(np.where(np.isfinite(y_vals), y_vals, 0)), 0)
        else:
            densities = np.zeros(len(y_vals))
    ...
    if y_col == "end psi":
        ax.set_ylim(0, 1)
        ax.set_ylabel('End PSI', fontsize=10)
    elif y_col == "delta psi":
        ax.set_ylim(-1, 1)
        ax.set_ylabel('ΔPSI', fontsize=10)
    elif y_col == "end logit psi":
        ax.set_ylim(-8, 8)
        ax.set_ylabel('End PSI (Logit)', fontsize=10)
    elif y_col == "delta logit psi":
        ax.set_ylim(-10, 10)
        ax.set_ylabel('ΔLogit PSI', fontsize=10)
    else:
